...
<?php
	$query1 = "SELECT `id`, `username`, `name`, `post` FROM `users` WHERE `user_role_id` = '7' ";
...
?>
				<h3>RECTOR DETAILS</h3>
				<form action="rector.php" name="rector" method="POST">
					<table width="500px" align="center" cellpadding="8">
						<tr>
							<td><label for="name">Name:- </label></td>
							<td><input type="text" onkeyup="valiname('rector','name');" name="name" id="name" value="<?php echo $row1['name']; ?>" readonly></td>
						</tr>
						<tr>
							<td><label for="post">Designation:- </label></td>
							<td><input type="text" name="post" onkeyup="validtitle('rector','post');" id="post" value="<?php echo $row1['post']; ?>" readonly></td>
						</tr>
						<tr>
