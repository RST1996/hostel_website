<?php
	...
	$page_name = "rights/viewdownloads.php";
	...
	if (isset($_REQUEST['del']) && isset($_REQUEST['path'])) {
		...
		$del_query = "DELETE FROM `downloads` WHERE `id` = '$delid'";
		if ($res = mysqli_query($dbcon,$del_query)) {
			if (mysqli_affected_rows($dbcon) > 0) {
				unlink("../../".$delpath);
				echo "<script> alert('Downloads Successfully deleted!!'); </script>";
				header('Location:viewdownloads.php');
			}
			else
			{
				echo "<script> alert('Downloads Not found!!'); </script>";
			}
		}
		else
		{
...
<body>
	...
	<div class="content_section">
		...
		<div class="main_content_display">
			...
			<h3>DOWNLOADS</h3>
<?php
	$sel_query = "SELECT `downloads`.`id`,`downloads`.`title`,`downloads`.`path`,`users`.`name` FROM `downloads` LEFT JOIN `users` ON `downloads`.`uploader_id` = `users`.`id`";
